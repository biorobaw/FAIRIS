...
class Goals(QWidget):
    ...
    def __init__(self, id, x, y, *args, **kwargs):
        super(Goals, self).__init__(*args, **kwargs)
    ...
    def setSelected(self, new_val: bool):
        ...
        if new_val != self.is_selected:
            ...
            if new_val:
                Goals.all_selected.add(self)
            elif self in Goals.all_selected:
                Goals.all_selected.remove(self)
    ...
    def clear_all_selected():
        aux = Goals.all_selected
        Goals.all_selected = set()
        for pc in aux:
            pc.setSelected(False)

    def delete(self):
        if self in Goals.all_selected:
            Goals.all_selected.remove(self)
    ...
    def fromstring(s):
        ...
        if len(args) == 3:
            return Goals(args[0], args[1], args[2])
        else:
