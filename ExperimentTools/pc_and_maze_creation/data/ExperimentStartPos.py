...
class ExperimentStartPos(QWidget):
    ...
    def __init__(self, x, y, w, *args, **kwargs):
        super(ExperimentStartPos, self).__init__(*args, **kwargs)
    ...
    def setSelected(self, new_val: bool):
        ...
        if new_val != self.is_selected:
            ...
            if new_val:
                ExperimentStartPos.all_selected.add(self)
            elif self in ExperimentStartPos.all_selected:
                ExperimentStartPos.all_selected.remove(self)
    ...
    def clear_all_selected():
        aux = ExperimentStartPos.all_selected
        ExperimentStartPos.all_selected = set()
        for pc in aux:
            pc.setSelected(False)

    def delete(self):
        if self in ExperimentStartPos.all_selected:
            ExperimentStartPos.all_selected.remove(self)
    ...
    def fromstring(s):
        ...
        if len(args) == 3:
            return ExperimentStartPos(args[0], args[1], args[2])
        else:
