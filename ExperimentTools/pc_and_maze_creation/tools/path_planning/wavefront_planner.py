...
def wave_front_planner(walls, feeders):
    ...
    for f_id, f in feeders.iterrows():
        goal_indices = goal_indices.union(
            get_goal_indeces(f.x, f.y, min_x, min_y, precision, num_x, num_y, feeding_distance, occupancy_grid,
                             possible_walls))
...
def wave_front_planner_from_pickles(wall_pickles, feeder_pickles):
    walls = pd.DataFrame(columns=['x1', 'y1', 'x2', 'y2'],
                         data=[[w.x1(), w.y1(), w.x2(), w.y2()] for w in wall_pickles])
    feeders = pd.DataFrame(columns=['x', 'y'], data=[[f.x(), f.y()] for f in feeder_pickles])
    return wave_front_planner(walls, feeders)


def wave_front_planner_from_file(maze_file, folder=None):
    print('Starting wavefront planner in ', maze_file)
    if folder == None:
        full_path = os.path.join(folder, maze_file)
    else:
        full_path = maze_file
    walls, feeders, start_positions = parse_maze_for_wavefront(full_path)
