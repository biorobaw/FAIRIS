...
#        option_termination = Bernoulli(termination).sample()
        ...
        next_option = Q.argmax(dim=-1)
        term_v = True
        if np.random.random() > self.term_val:
            term_v = False
        return term_v, next_option.item()
    
    def get_terminations(self, state):
#        terms = torch.ones((state.shape[0], self.num_options)).to(self.device)
        terms = torch.full((state.shape[0], self.num_options), self.term_val).to(self.device)
        return terms
#        return self.terminations(state).sigmoid() 
