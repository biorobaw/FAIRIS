...
class Logger():
    ...
    def log_episode(self, steps, reward, option_lengths, ep_steps, epsilon, current_goal, loss):
        self.n_eps += 1
        logging.info(f"> ep {self.n_eps} done. total_steps={steps} | r={reward} | episode_steps={ep_steps} "\
            f"| hs={(time.time()-self.start_time) / 60 / 60:.3f} | eps={epsilon:.3f} " \
            f"| cur_goal={current_goal} | loss={loss:.3f}")
#            f"\noption_lengths={np.mean(option_lengths[0])}, {np.mean(option_lengths[1])}, {np.mean(option_lengths[2])}, {np.mean(option_lengths[3])} | current_goal={current_goal}")
