...
def find_python_version():
    # Only accept Python 3.11
    preferred_order = ["python3.11"]
...
def create_venv(python_executable):
    try:
        subprocess.check_call([python_executable, "-m", "venv", "fairis_venv"])
...
def add_fairis_to_path(project_path):
    if platform.system() == "Windows":
        site_packages_dir = os.path.join("fairis_venv", "Lib", "site-packages")
    else:
        python_bin = os.path.join("fairis_venv", "bin", "python3")
        if not os.path.exists(python_bin):
            python_bin = os.path.join("fairis_venv", "bin", "python")  # fallback
        ...
        try:
            ...
            print(f"Failed to detect fairis_venv Python version: {e}")
            sys.exit(1)

        site_packages_dir = os.path.join("fairis_venv", "lib", version_tag, "site-packages")
